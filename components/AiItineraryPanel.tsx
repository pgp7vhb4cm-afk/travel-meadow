...
export default function AiItineraryPanel({
  ...
  async function generate() {
    ...
    try {
      ...
      if (!response.ok) {
        setError(data.error || "Something went wrong while building your itinerary.");
        return;
      }
      setResult(data);
    } catch {
  ...
  if (!result && !loading) {
    return (
      <div className="bg-meadow-light border border-meadow/20 rounded-2xl p-5 flex flex-col sm:flex-row items-start sm:items-center gap-4 justify-between">
        <div>
          <h3 className="flex items-center gap-2 text-sm font-medium text-meadow-dark mb-1">
            <Sparkles size={16} aria-hidden="true" />
            Build me a tailored itinerary
          </h3>
          <p className="text-sm text-meadow-dark/80">
            We&apos;ll research current, real recommendations for {destinationName} and put
            together a day-by-day plan based on your answers.
          </p>
        </div>
        <button
          onClick={generate}
          className="inline-flex items-center gap-2 bg-meadow text-white text-sm px-4 py-2.5 rounded-md hover:bg-meadow-dark transition-colors whitespace-nowrap"
        >
          <Sparkles size={16} aria-hidden="true" />
          Generate itinerary
        </button>
  ...
  if (loading) {
    return (
      <div className="bg-meadow-light border border-meadow/20 rounded-2xl p-6 text-center">
        ...
        <p className="text-sm text-meadow-dark">{LOADING_MESSAGES[messageIndex]}</p>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="bg-red-50 border border-red-200 rounded-2xl p-5">
        <p className="text-sm text-red-700 flex items-center gap-1.5 mb-3">
          <AlertCircle size={14} aria-hidden="true" /> {error}
        </p>
        <button
          onClick={generate}
          className="text-sm text-meadow hover:text-meadow-dark underline"
        >
          Try again
        </button>
      </div>
    ...
    <div className="border border-gray-200 rounded-2xl p-5 sm:p-6">
      <div className="flex items-center justify-between gap-2 mb-3 flex-wrap">
        <h3 className="flex items-center gap-2 text-base font-medium text-gray-900">
          <Sparkles size={17} className="text-meadow" aria-hidden="true" />
          Your tailored itinerary
        </h3>
        {source === "ai" ? (
          <span className="inline-flex items-center gap-1.5 text-xs bg-meadow-light text-meadow-dark border border-meadow/30 rounded-full px-2.5 py-1">
            AI-researched, web-grounded
          </span>
        ) : (
          <span className="inline-flex items-center gap-1.5 text-xs bg-gray-100 text-gray-500 border border-gray-200 rounded-full px-2.5 py-1">
            Sample outline — add an Anthropic API key for a real, researched itinerary
          </span>
        )}
      </div>
      ...
      {itinerary.bestTimeToVisit && (
        <p className="text-sm text-gray-500 mb-4">
          <span className="font-medium text-gray-700">Best time to go: </span>
          {itinerary.bestTimeToVisit}
        </p>
      )}
      ...
      {itinerary.days.length > 0 && (
        <div className="mb-5">
          <h4 className="flex items-center gap-1.5 text-sm font-medium text-gray-900 mb-2">
            <CalendarDays size={14} className="text-meadow" aria-hidden="true" />
            Day by day
          </h4>
          <div className="flex flex-col gap-2">
            {itinerary.days.map((day) => (
              <div key={day.day} className="flex gap-3 border-l-2 border-meadow/30 pl-3 py-0.5">
                <div className="text-xs font-medium text-meadow whitespace-nowrap pt-0.5">
                  Day {day.day}
                </div>
                <div>
          ...
          <div className="flex flex-col gap-1">
            {itinerary.sources.map((source) => (
              <a
                ...
                className="inline-flex items-center gap-1.5 text-xs text-gray-500 hover:text-meadow transition-colors"
              >
